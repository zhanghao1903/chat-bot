...
限次作家效应器、当轮权威时间与时效核验、当前群上下文工具，以及需管理员公开启用的成员认识。
...
时间感知功能把模型调用硬上限提高到 12，以容纳两个互不借用额度的只读工具预算及一次
不计工具额度的回答时区选择：既有群上下文
仍为普通 3 次、复杂轮次最多 5 次；Tavily Web 工具独立最多 5 次。两类工具共享整个 effect
截止时间、16 KiB 已接纳结果上限和最终回复保留位，因此理论上的 `1 + 5 + 5 + 1` 不是最低
调用次数，也不是无限 loop。

## 当前时间与时效信息

每次 Writer 模型调用前，应用都会重新采集 UTC 和回答目标 IANA 时区的当前时间，并提供
本地 RFC3339 时间、UTC 偏移、日期、星期及采集时刻。上下文 ID 同时绑定 effect 和模型
调用序号，因此不能复用上一轮、同秒的另一调用或另一条消息的快照。默认回答时区沿用当前
群的自动化配置；没有配置时使用 `Asia/Shanghai`。Writer 可以在当前轮恰好一次选择明确、
无歧义的其他 IANA 时区，该选择不占上下文工具或 Web 工具额度。

当前消息和近期场景同时携带其实际 UTC 发生时间及按回答时区换算的本地时间。scheduled
场景把原计划日期、餐次和时区与当前执行时间分开呈现。是否需要实时 Web 核验由 Writer
根据完整语义决定，应用不使用自然语言关键词或正则代替该判断。已核实的时效回答只能引用
本轮 Web 结果 ID，最终由应用附加回答时区下的“截至”时间和 1–3 个来源；无法核实时会使用
固定的不确定性边界，不把模型记忆冒充实时事实。时间样本和最终模式写入迁移 7 的有界审计
表，不保存完整提示、网页正文或秘密。
...
| `EFFECT_MAX_MODEL_CALLS` | 可选 | `12`，范围 `1..12` | 每次效应器最大模型调用，含一次时区选择与最终回复保留位 |
