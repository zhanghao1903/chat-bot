...
第一阶段在现有单群 Telegram 长轮询服务上增加三个相互分离的能力：

1. **Character Bundle**：装载一份用户已确认、不可变且可回滚的原创 Character Bible。
2. **Member Recognition System**：异步观察当前公开群消息，形成群隔离、可修订、可重置的成员认识。
3. **Writer Effector**：读取人格、当前场景、相关成员认识和受控工具结果，最多提交一个外部效果。
...
flowchart TD
    ...
    IN --> TR["TriggerPolicy"]
    TR -->|ignore| STOP["No effect"]
    TR -->|direct or contextual candidate| CA["ContextAssembler"]
    CA --> CB["CharacterBundleLoader"]
    CA --> MM["MemberMemoryRepository"]
...
| `TriggerPolicy` | 直接触发识别、情境候选硬门限、控制命令路由。 | 写回复、调用工具或更新长期认识。 |
| `CharacterBundleLoader` | Schema、版本、内容 digest 和启用清单校验。 | 静默补充角色内容或在线改写人格。 |
| `ContextAssembler` | 最近场景和最小必要成员认识的确定性装配。 | 写认识或越过当前群读取数据。 |
...
“作家”是 Character Bible、认识规则和效应器共同实现的产品视角，不要求新增一个名为
`Writer` 的独立长期状态组件。
...
### 4.2 TriggerDecision And EffectRequest
...
Direct triggers are bot mentions, replies to a bot-authored message, and non-control bot
commands. A direct trigger bypasses contextual cadence but never bypasses group, sender,
delivery, safety or total outbound-rate limits.
...
Passing the hard gate only creates a candidate. `WriterEffector` may still return `silence`
when relevance, context, confidence or participation value is insufficient.
...
MemberMemoryItem
  ...
  persona_version?         # required for subjective preference
  revision
  source_refs[]
```

Facts, observations and impressions are not interchangeable. Subjective preferences always
carry the persona version that owns that point of view and cannot be rendered as objective
facts. Every item has at least one validated source reference.
...
| `recognition_jobs` | Unique source job; subject, pending/leased/retry/dead status, attempt count, lease, next attempt, policy version. |
...
sequenceDiagram
    ...
    participant R as Recognition queue
    participant G as TriggerPolicy
    ...
    else direct/contextual candidate
...
2. Effector context uses the last committed recognition snapshot; the current message is not
   allowed to rewrite memory before its own reply.
3. Recognition processing is independent and may finish before or after later messages.
4. A recognition failure never changes the effect outcome and never stops long polling.

## 7. Writer Context And Tools

### 7.1 Deterministic Default Context

`ContextAssembler` includes without a tool call:
...
prompt receives bounded memory views and source metadata, not unrestricted database rows.
...
One invalid structured model response may be repaired only if another one of the three calls
remains. There is no retry outside these limits. No second model is used as an online
personality judge in phase one.
...
### 8.1 Final Validation And Degradation
...
3. load the Character Bundle's relationship/preference rules;
...
- a subjective preference is labeled as fact or lacks the active persona version;
...
| --- | --- |
| Character Bundle schema, digest, immutable version and injection tests | REQ-001–006; AC-001–004 |
| Telegram normalization, direct/context trigger and cadence tests | REQ-007–012; AC-003–004 |
...
| Recognition job, proposal validator, revision and decay tests | REQ-021–027; AC-009–013 |
...
| --- | --- |
| REQ-001–006 | Character Bundle contract, prompt precedence, activation and evaluation gates. |
| REQ-007–012 | Trigger contracts, bounded effector, one external effect and degradation. |
| REQ-013–020 | Deterministic context, read-only tools, budgets and audit. |
| REQ-021–027 | Durable independent recognition worker, typed memory and revision/decay. |
...
- **D-005**：认识系统和效应器是独立写/读边界；当前消息的认识更新不影响同一次回复。
- **D-006**：最近 20 条消息与关键成员认识为确定性默认上下文，不消耗工具预算。
- **D-007**：首发只提供成员认识查询和七日群消息查询两个本地只读工具。
- **D-008**：效应器最多三次模型调用、两次工具调用，第三次强制禁用工具，总 deadline 20 秒。
- **D-009**：人格门禁主要由版本化 bundle 和离线评测承担；线上不增加第二个评分模型。
- **D-010**：认识 worker 每个 job 最多一次模型调用，失败持久化重试三次，不阻塞 polling/effector。
- **D-011**：认识项以事实、观察、印象、共同经历和主观偏好分类，并在读取时计算衰减置信度。
- **D-012**：主观偏好绑定 persona version；人格切换不会直接继承旧角色的偏好。
- **D-013**：持久认识默认关闭，只有管理员成功发布公开说明后才能启用。
- **D-014**：reset barrier 同时阻止排队和并发中的旧任务恢复被清除认识。
- **D-015**：所有 feature 外部效果以 `(chat_id, trigger_event_id)` 唯一，版本切换不能重复发送。
- **D-016**：继续选择 at-most-once；发送结果不确定时不自动重试。
- **D-017**：以 `fixed -> persona_direct -> persona_full` 分阶段启用，固定回复模式是运行时回滚路径。
- **D-018**：模型厂商适配器延后到 F3 选择，但必须实现本文的 provider-neutral structured port。
